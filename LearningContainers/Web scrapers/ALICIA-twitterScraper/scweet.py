...
from utils import init_driver, get_last_date_from_csv, log_search_page, log_search_page_url, keep_scroling
...
def scrap(start_date, max_date, words=None, to_account=None, from_account=None, interval=5, lang=None,
    ...
    if words:
        ...
        path = save_dir + "/" + str(init_date).split(' ')[0] + '_' + \
               str(max_date).split(' ')[0] + '.csv'
    elif from_account:
        #path = save_dir + "/" + from_account + '_' + str(init_date).split(' ')[0] + '_' + str(max_date).split(' ')[
        #    0] + '.csv'
        path = save_dir + "/" + from_account[0] + '_' + str(init_date).split(' ')[0] + '_' + str(max_date).split(' ')[
            0] + '.csv'
    elif to_account:
    ...
    with open(path, write_mode, newline='', encoding='utf-8') as f:
        ...
        while end_date <= datetime.datetime.strptime(max_date, '%Y-%m-%d'):
            ...
            print("looking for tweets between " + str(start_date) + " and " + str(end_date) + " ...", flush=True)

            # start scrolling and get tweets
            tweet_parsed = 0

            driver, data, writer, tweet_ids, scrolling, tweet_parsed, scroll, last_position = \
                keep_scroling(driver, data, tweet_ids, scrolling, last_position, writer, tweet_parsed, limit, scroll)
...
def scrape_url(url, root_path, headless=True, limit=float("inf"), display_type="Top", proxy=None):
    # initiate the driver
    driver = init_driver(headless, proxy)
    ...
    tweet_ids = set()
    #save_dir = "outputs"
    save_dir = os.path.join(root_path, 'outputs')
    ...
    full_output_path = os.path.join(save_dir, 'full_output.csv')
    output_path = os.path.join(save_dir, 'output.csv')

    with open(full_output_path, write_mode, newline='', encoding='utf-8') as f:
        header = ['UserScreenName', 'UserName', 'Timestamp', 'Text', 'Emojis', 'Comments', 'Likes', 'Retweets',
                  'Image link', 'Tweet URL']
        writer = csv.writer(f)
        if write_mode == 'w':
        ...
        # log search page between start_date and end_date
        log_search_page_url(driver, url, display_type)
        ...
        driver, data, writer, tweet_ids, scrolling, tweet_parsed, scroll, last_position = \
            keep_scroling(driver, images_path, data, tweet_ids, scrolling, last_position, writer, tweet_parsed, limit, scroll)

        f.close()

    # Create condensed output (output.csv) to pass to SENTIR
    with open(output_path, write_mode, newline='', encoding='utf-8') as g:
        header = ['Text']
        writer = csv.writer(g)
        if write_mode == 'w':
            writer.writerow(header)
